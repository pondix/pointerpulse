...
std::vector<uint8_t> build_format_description_crc() {
    uint32_t payload_len = 2 + 50 + 4 + 1 + 1 + 1; // version + server + ts + header_len + one type len + alg byte
    uint32_t event_size = 19 + payload_len + 4;     // include CRC32 trailer
    std::vector<uint8_t> data(19 + payload_len, 0);
    ...
    write32(0, 1);
    data[4] = static_cast<uint8_t>(EventType::FORMAT_DESCRIPTION_EVENT);
    write32(5, 1);
    ...
    size_t p = 19;
    write16(p, 4); // binlog version
    p += 2;
    const std::string server = "8.0.30-log";
    std::copy(server.begin(), server.end(), data.begin() + p);
    p += 50;
    write32(p, 0);
    p += 4;
    data[p++] = 19;      // header length
    data[p++] = 19;      // first type header length entry
    data[p++] = 0x01;    // checksum algorithm CRC32

    append_crc(data);
    return data;
}

std::vector<uint8_t> build_query_event_crc(const std::string &schema, const std::string &query) {
    uint32_t payload_len = 4 + 4 + 1 + 2 + 2 + schema.size() + 1 + query.size();
    uint32_t event_size = 19 + payload_len + 4; // include CRC
    std::vector<uint8_t> data(19 + payload_len, 0);
    auto write32 = [&](size_t offset, uint32_t v) {
        data[offset] = v & 0xff;
        data[offset + 1] = (v >> 8) & 0xff;
...
int main() {
    ...
    auto fmt = build_format_description_crc();
    bool ok = parser.parse_event(fmt, evt);
    assert(ok);
    assert(evt.format_desc.has_value());
    assert(evt.format_desc->checksum == ChecksumAlgorithm::CRC32);

    evt = BinlogEvent{};
    auto query_evt = build_query_event_crc("testdb", "INSERT INTO t VALUES (1)");
    ok = parser.parse_event(query_evt, evt);
    assert(ok);
    assert(evt.query.has_value());
    assert(evt.query->query.find("INSERT") != std::string::npos);
